...
Tests            152 cases, run on all of the above in CI. The Windows
...
| 152 tests | 48 encoding cases, 64 editing cases, 37 syntax colouring cases and 3 that read the escapes the terminal is actually sent, driven through a real pty. Every push runs them on five operating systems. |
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 152 tests
```
...
./scripts/test-syntax.sh   src/jvim3   # 37 cases: what syntax/ actually colours
./scripts/test-sgr.sh      src/jvim3   # 3 cases: the escapes a terminal is sent
```

Both drive the editor through a real pty (`scripts/ptyrun.c`) and compare bytes,
so they exercise the same input path a person does. Each case is given 20
