...
Tests            225 cases, run on all of the above in CI. The Windows
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 225 tests
```
...
./scripts/test-hostile.sh  src/jvim3   # 17 cases: input nobody intended
```
...
Every push and pull request builds and runs all 225 on Linux, FreeBSD, NetBSD,
