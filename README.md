...
Tests            163 cases, run on all of the above in CI. The Windows
...
| 163 tests | 48 encoding cases, 64 editing cases, 44 syntax colouring cases and 7 that read the escapes the terminal is actually sent, driven through a real pty. Every push runs them on five operating systems. |
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 163 tests
```
...
./scripts/test-syntax.sh   src/jvim3   # 44 cases: what syntax/ actually colours
./scripts/test-sgr.sh      src/jvim3   # 7 cases: the escapes a terminal is sent
```
