...
Tests            343 cases, run on all of the above in CI, plus 6 that run
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 343 tests
...
./scripts/test-encoding.sh src/jvim3   # 107 cases: encodings, multi-byte editing
