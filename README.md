...
Tests            177 cases, run on all of the above in CI. The Windows
...
| 177 tests | 48 encoding cases, 72 editing cases, 50 syntax colouring cases and 7 that read the escapes the terminal is actually sent, driven through a real pty. Every push runs them on five operating systems. |
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 177 tests
```
