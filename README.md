...
Tests            322 cases, run on all of the above in CI, plus 6 that run
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 322 tests
...
./scripts/test-encoding.sh src/jvim3   # 86 cases: encodings, multi-byte editing
