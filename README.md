...
Tests            351 cases, run on all of the above in CI, plus 6 that run
                 the Windows executables there. The Windows keyboard has 17
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 351 tests
...
./scripts/test-encoding.sh src/jvim3   # 115 cases: encodings, multi-byte editing
./scripts/test-editing.sh  src/jvim3   # 121 cases: motions, operators, registers,
