...
Tests            221 cases, run on all of the above in CI. The Windows
...
| 221 tests | 48 encoding cases, 74 editing cases, 75 syntax colouring cases, 9 that read the escapes the terminal is actually sent, and 15 that hand it input nobody intended — 2 MB on one line, every byte value there is, a multi-byte sequence cut in half by the end of the file. All driven through a real pty. Every push runs them on five operating systems, and again under AddressSanitizer and UndefinedBehaviorSanitizer. |
...
cd jvim3
./scripts/build-unix.sh test        # build src/jvim3, then run the 221 tests
```
...
./scripts/test-editing.sh  src/jvim3   # 74 cases: motions, operators, registers,
...
./scripts/test-hostile.sh  src/jvim3   # 15 cases: input nobody intended
```
...
Every push and pull request builds and runs all 221 on Linux, FreeBSD, NetBSD,
