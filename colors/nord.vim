" Nord theme for JVim
set background=dark
hi clear
let g:colors_name = "nord"

hi Comment      guifg=#616e88
hi Constant     guifg=#b48ead
hi String       guifg=#a3be8c
hi Number       guifg=#b48ead
hi Identifier   guifg=#88c0d0
hi Function     guifg=#88c0d0
hi Statement    guifg=#81a1c1
hi Conditional  guifg=#81a1c1
hi Repeat       guifg=#81a1c1
hi Label        guifg=#81a1c1
hi Operator     guifg=#81a1c1
hi Keyword      guifg=#81a1c1
hi Exception    guifg=#81a1c1
hi PreProc      guifg=#81a1c1
hi Include      guifg=#81a1c1
hi Define       guifg=#81a1c1
hi Macro        guifg=#81a1c1
hi PreCondit    guifg=#81a1c1
hi Type         guifg=#8fbcbb gui=bold
hi StorageClass guifg=#8fbcbb
hi Structure    guifg=#8fbcbb
hi Typedef      guifg=#8fbcbb
hi Special      guifg=#ebcb8b
hi Tag          guifg=#ebcb8b
hi Error        guifg=#eceff4 guibg=#bf616a gui=bold
hi Todo         guifg=#2e3440 guibg=#ebcb8b gui=bold
hi MdHead       guifg=#88c0d0 gui=bold
hi MdCode       guibg=#3b4252
hi DiffAdd      guifg=#a3be8c guibg=#2f3d44
hi DiffDel      guifg=#bf616a guibg=#43313a
hi HtmlEmbed    guibg=#3b4252
