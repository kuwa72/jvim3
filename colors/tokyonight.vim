" Tokyo Night theme for JVim
set background=dark
hi clear
let g:colors_name = "tokyonight"

hi Normal       guifg=#c0caf5 guibg=#1a1b26
hi Comment      guifg=#565f89
hi Constant     guifg=#ff9e64
hi String       guifg=#9ece6a
hi Number       guifg=#ff9e64
hi Identifier   guifg=#7dcfff
hi Function     guifg=#7aa2f7
hi Statement    guifg=#bb9af7
hi Conditional  guifg=#bb9af7
hi Repeat       guifg=#bb9af7
hi Label        guifg=#bb9af7
hi Operator     guifg=#bb9af7
hi Keyword      guifg=#bb9af7
hi Exception    guifg=#bb9af7
hi PreProc      guifg=#7dcfff
hi Include      guifg=#7dcfff
hi Define       guifg=#7dcfff
hi Macro        guifg=#7dcfff
hi PreCondit    guifg=#7dcfff
hi Type         guifg=#2ac3de gui=bold
hi StorageClass guifg=#2ac3de
hi Structure    guifg=#2ac3de
hi Typedef      guifg=#2ac3de
hi Special      guifg=#e0af68
hi Tag          guifg=#e0af68
hi Error        guifg=#c0caf5 guibg=#f7768e gui=bold
hi Todo         guifg=#1a1b26 guibg=#e0af68 gui=bold
hi MdHead       guifg=#7aa2f7 gui=bold
hi MdCode       guibg=#24283b
hi DiffAdd      guifg=#9ece6a guibg=#203038
hi DiffDel      guifg=#f7768e guibg=#37222c
hi HtmlEmbed    guibg=#24283b
