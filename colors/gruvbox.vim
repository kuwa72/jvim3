" Gruvbox theme for JVim
set background=dark
hi clear
let g:colors_name = "gruvbox"

hi Comment      guifg=#928374
hi Constant     guifg=#d3869b
hi String       guifg=#b8bb26
hi Number       guifg=#d3869b
hi Identifier   guifg=#83a598
hi Function     guifg=#b8bb26
hi Statement    guifg=#fb4934
hi Conditional  guifg=#fb4934
hi Repeat       guifg=#fb4934
hi Label        guifg=#fb4934
hi Operator     guifg=#fb4934
hi Keyword      guifg=#fb4934
hi Exception    guifg=#fb4934
hi PreProc      guifg=#8ec07c
hi Include      guifg=#8ec07c
hi Define       guifg=#8ec07c
hi Macro        guifg=#8ec07c
hi PreCondit    guifg=#8ec07c
hi Type         guifg=#fabd2f gui=bold
hi StorageClass guifg=#fabd2f
hi Structure    guifg=#fabd2f
hi Typedef      guifg=#fabd2f
hi Special      guifg=#fe8019
hi Tag          guifg=#fe8019
hi Error        guifg=#fbf1c7 guibg=#cc241d gui=bold
hi Todo         guifg=#282828 guibg=#fabd2f gui=bold
hi MdHead       guifg=#fabd2f gui=bold
hi MdCode       guibg=#3c3836
hi DiffAdd      guifg=#b8bb26 guibg=#32361a
hi DiffDel      guifg=#fb4934 guibg=#3c1f1e
hi HtmlEmbed    guibg=#3c3836
