...
./scripts/build-unix.sh test     # ビルドしてテスト 337 ケースを実行
...
インストールするのは上記のパッケージ名です。`mingw-w64` メタパッケージや Homebrew 版は UCRT 対象なので使えません。
...
./scripts/test-encoding.sh src/jvim3   # 101 ケース
./scripts/test-editing.sh  src/jvim3   # 121 ケース

./scripts/test-syntax.sh   src/jvim3   # 88 ケース
...
以上）。`scripts/test-sgr.sh` は端末に実際に送られるエスケープを見ます。syntax
ルールの内容を確認するスイートではありません。描画側を直接見る唯一のスイートです。`scripts/test-hostile.sh` は誰も意図して
いない入力と終わり方を与えます — 1 行 2 MB、あらゆるバイト値、ファイル末尾で
切れたマルチバイト列、再帰的なマッチャを使い切るほど入れ子にした正規表現、
編集中にセッションが切れる。合わせて 337 ケースです。


敵性入力スイートは、何もクラッシュしていないのにケースを失敗として報告できる
唯一のスイートです。エディタがそのケースを終えられないとき KNOWN-FAIL になり、
「使えないほど遅くなった」をテストが言える唯一の形です。入力ファイルは
`scripts/hostilegen.c` で作ります。awk や dd では、5 つの OS
ごとの awk がゼロバイトの扱いで一致しないからです。最後の 2 件は `PTYRUN_SIGNAL`
...
- Linux (Ubuntu 24.04 / gcc)、FreeBSD (clang)、NetBSD、OpenBSD、DragonFly で
  230 テスト全件通過。Linux では AddressSanitizer と
...
- 実機・実端末・本物の IME での確認はしていません。すべてシリアルコンソール、
  pty、CI ランナー上です。
- Windows 版は CI で実行されるようになりました。`scripts/test-winrun.sh` が
  スクリプト入力で 6 ケースです。
  キーを打つテスト (`scripts/test-winkeys.sh`) は Windows 実機が必要で手動です。
