...
テスト         312 ケース。上記すべてで CI が実行。Windows のキー入力は
...
| 312 個のテスト | エンコーディング 76 ケース、編集 121 ケース、シンタックスカラー 88 ケース、端末に実際に送られるエスケープを見る 9 ケース、そして誰も意図していない入力や終わり方を与える 18 ケース（1 行 2 MB、あらゆるバイト値、ファイル末尾で切れたマルチバイト列、編集中にセッションが切れる）。いずれも本物の pty 越しです。push ごとに 5 つの OS で実行され、さらに AddressSanitizer と UndefinedBehaviorSanitizer の下でも走ります。 |
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 312 個のテストを実行
```
...
./scripts/test-encoding.sh src/jvim3   # 76 ケース: 文字コード、マルチバイト編集
...
環境で 312 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
