...
テスト         227 ケース。上記すべてで CI が実行。Windows のキー入力は
...
| 227 個のテスト | エンコーディング 50 ケース、編集 76 ケース、シンタックスカラー 75 ケース、端末に実際に送られるエスケープを見る 9 ケース、そして誰も意図していない入力や終わり方を与える 17 ケース（1 行 2 MB、あらゆるバイト値、ファイル末尾で切れたマルチバイト列、編集中にセッションが切れる）。いずれも本物の pty 越しです。push ごとに 5 つの OS で実行され、さらに AddressSanitizer と UndefinedBehaviorSanitizer の下でも走ります。 |
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 227 個のテストを実行
```
...
Windows 版はどちらも CI に実行時テストがなく、コンパイルと、同じ移植部分の
ソースを Unix でテストすることで代えています。Windows のコンソールモードでの
日本語入力は不安定、GDI はカラー絵文字を描かず、一部の文字コード変換は
一方向です。全項目とそれぞれの実際の影響は
...
./scripts/test-encoding.sh src/jvim3   # 50 ケース: 文字コード、マルチバイト編集
./scripts/test-editing.sh  src/jvim3   # 76 ケース: 移動、オペレータ、レジスタ、
                                       #   マーク、undo、ex の範囲指定、:g、:s、:!
./scripts/test-syntax.sh   src/jvim3   # 75 ケース: syntax/ が実際に何を色付けするか
./scripts/test-sgr.sh      src/jvim3   # 9 ケース: 端末に実際に送られるエスケープ
./scripts/test-hostile.sh  src/jvim3   # 17 ケース: 誰も意図していない入力
```
...
環境で 227 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
