...
テスト         152 ケース。上記すべてで CI が実行。Windows のキー入力は
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 152 個のテストを実行
```
...
./scripts/test-sgr.sh      src/jvim3   # 3 ケース: 端末に実際に送られるエスケープ
```
...
環境で 152 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
