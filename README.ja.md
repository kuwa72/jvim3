...
テスト         136 ケース。上記すべてで CI が実行。Windows のキー入力は
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 136 個のテストを実行
```
...
./scripts/test-syntax.sh   src/jvim3   # 24 ケース: syntax/ が実際に何を色付けするか
```
...
環境で 136 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
