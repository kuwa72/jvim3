...
テスト         217 ケース。上記すべてで CI が実行。Windows のキー入力は
...
| 217 個のテスト | エンコーディング 48 ケース、編集 72 ケース、シンタックスカラー 73 ケース、端末に実際に送られるエスケープを見る 9 ケース、そして誰も意図していない入力を与える 15 ケース（1 行 2 MB、あらゆるバイト値、ファイル末尾で切れたマルチバイト列）。いずれも本物の pty 越しです。push ごとに 5 つの OS で実行され、さらに AddressSanitizer と UndefinedBehaviorSanitizer の下でも走ります。 |
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 217 個のテストを実行
```
...
./scripts/test-editing.sh  src/jvim3   # 72 ケース: 移動、オペレータ、レジスタ、
...
環境で 217 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
