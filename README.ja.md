...
テスト         176 ケース。上記すべてで CI が実行。Windows のキー入力は
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 176 個のテストを実行
```
...
./scripts/test-syntax.sh   src/jvim3   # 49 ケース: syntax/ が実際に何を色付けするか
...
環境で 176 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
