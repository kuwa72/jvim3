...
テスト         219 ケース。上記すべてで CI が実行。Windows のキー入力は
...
| 219 個のテスト | エンコーディング 48 ケース、編集 74 ケース、シンタックスカラー 73 ケース、端末に実際に送られるエスケープを見る 9 ケース、そして誰も意図していない入力を与える 15 ケース（1 行 2 MB、あらゆるバイト値、ファイル末尾で切れたマルチバイト列）。いずれも本物の pty 越しです。push ごとに 5 つの OS で実行され、さらに AddressSanitizer と UndefinedBehaviorSanitizer の下でも走ります。 |
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 219 個のテストを実行
```
...
./scripts/test-editing.sh  src/jvim3   # 74 ケース: 移動、オペレータ、レジスタ、
                                       #   マーク、undo、ex の範囲指定、:g、:s、:!
./scripts/test-syntax.sh   src/jvim3   # 73 ケース: syntax/ が実際に何を色付けするか
./scripts/test-sgr.sh      src/jvim3   # 9 ケース: 端末に実際に送られるエスケープ
./scripts/test-hostile.sh  src/jvim3   # 15 ケース: 誰も意図していない入力
```
...
環境で 219 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
