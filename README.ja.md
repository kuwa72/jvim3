...
テスト         179 ケース。上記すべてで CI が実行。Windows のキー入力は
...
| 110 個のテスト | エンコーディング 46 ケースと編集 72 ケース。実際の pty 越しに動かします。push ごとに 5 つの OS で実行されます。 |
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 179 個のテストを実行
```
...
リリースしている Windows 版は 32 ビットで、64 ビット版は CI の外では実行された
ことがありません。Windows のコンソールモードでの日本語入力は不安定、GDI は
カラー絵文字を描かず、一部の文字コード変換は一方向です。全項目とそれぞれの
実際の影響は [USAGE.ja.md](USAGE.ja.md#既知の制限) にまとめてあります。
...
./scripts/test-syntax.sh   src/jvim3   # 50 ケース: syntax/ が実際に何を色付けするか
...
環境で 179 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
