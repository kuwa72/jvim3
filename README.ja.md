...
テスト         202 ケース。上記すべてで CI が実行。Windows のキー入力は
...
| 202 個のテスト | エンコーディング 48 ケース、編集 72 ケース、シンタックスカラー 73 ケース、端末に実際に送られるエスケープを見る 9 ケース。push ごとに 5 つの OS で実行されます。 |
...
cd jvim3
./scripts/build-unix.sh test        # src/jvim3 をビルドして 202 個のテストを実行
```
...
環境で 202 ケースすべてを実行し、Windows 版を両アーキテクチャでクロスビルド
