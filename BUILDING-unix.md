...
the `:!` filter and wildcard expansion), `scripts/test-syntax.sh` (48 cases
— what the rules in `syntax/` actually colour, read back with `:syntax dump`,
one for every file in `syntax/`) and `scripts/test-sgr.sh` (7 cases — the
escapes the terminal is actually sent, which is the only check of the painter
rather than the rules). 167 cases in all.
...
- `/bin/sh` being dash
- All 167 tests, and the same again under AddressSanitizer
