...
`test` runs all five suites: `scripts/test-encoding.sh` (70 cases — kanji,
...
matcher, and the session dropping mid-edit). 306 cases in all.
...
- `/bin/sh` being dash
- All 306 tests, and the same again under both sanitizers
...
- All 306 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
