...
`test` runs all five suites: `scripts/test-encoding.sh` (115 cases — kanji,
...
matcher, and the session dropping mid-edit). 351 cases in all.
...
- `/bin/sh` being dash
- All 351 tests, and the same again under both sanitizers
...
- All 351 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
