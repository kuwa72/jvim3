...
`test` runs all five suites: `scripts/test-encoding.sh` (55 cases — kanji,
...
matcher, and the session dropping mid-edit). 291 cases in all.
...
- `/bin/sh` being dash
- All 291 tests, and the same again under both sanitizers
...
- All 291 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
