...
`test` runs all five suites: `scripts/test-encoding.sh` (130 cases — kanji,
...
matcher, and the session dropping mid-edit). 366 cases in all.
...
- `/bin/sh` being dash
- All 366 tests, and the same again under both sanitizers
...
- All 366 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
