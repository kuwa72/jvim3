...
`test` runs all five suites: `scripts/test-encoding.sh` (86 cases — kanji,
...
matcher, and the session dropping mid-edit). 322 cases in all.
...
- `/bin/sh` being dash
- All 322 tests, and the same again under both sanitizers
...
- All 322 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
