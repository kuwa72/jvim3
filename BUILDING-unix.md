...
`test` runs all five suites: `scripts/test-encoding.sh` (76 cases — kanji,
...
matcher, and the session dropping mid-edit). 312 cases in all.
...
- `/bin/sh` being dash
- All 312 tests, and the same again under both sanitizers
...
- All 312 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
