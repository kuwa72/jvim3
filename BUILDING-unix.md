...
`test` runs all five suites: `scripts/test-encoding.sh` (122 cases — kanji,
...
matcher, and the session dropping mid-edit). 358 cases in all.
...
- `/bin/sh` being dash
- All 358 tests, and the same again under both sanitizers
...
- All 358 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
