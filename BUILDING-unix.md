...
matcher, and the session dropping mid-edit). 249 cases in all.
...
- `/bin/sh` being dash
- All 249 tests, and the same again under both sanitizers
...
- All 249 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
