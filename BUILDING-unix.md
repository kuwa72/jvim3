...
matcher, and the session dropping mid-edit). 287 cases in all.
...
- `/bin/sh` being dash
- All 287 tests, and the same again under both sanitizers
...
- All 287 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
