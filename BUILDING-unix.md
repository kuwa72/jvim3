...
`test` runs all five suites: `scripts/test-encoding.sh` (143 cases — kanji,
...
matcher, and the session dropping mid-edit). 379 cases in all.
...
- `/bin/sh` being dash
- All 379 tests, and the same again under both sanitizers
...
- All 379 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
