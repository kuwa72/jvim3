...
`test` runs all five suites: `scripts/test-encoding.sh` (107 cases — kanji,
...
matcher, and the session dropping mid-edit). 343 cases in all.
...
- `/bin/sh` being dash
- All 343 tests, and the same again under both sanitizers
...
- All 343 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
