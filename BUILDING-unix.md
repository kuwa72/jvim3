...
`test` runs all five suites: `scripts/test-encoding.sh` (51 cases — kanji,
UTF-8, multi-byte editing, file names), `scripts/test-editing.sh` (78 cases —
motions, operators, registers, marks, undo, ex ranges, `:g`, `:s`, searching,
the `:!` filter and wildcard expansion), `scripts/test-syntax.sh` (79 cases
...
matcher, and the session dropping mid-edit). 235 cases in all.
...
- `/bin/sh` being dash
- All 235 tests, and the same again under both sanitizers
...
- All 235 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
