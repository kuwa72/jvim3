...
UTF-8, multi-byte editing, file names), `scripts/test-editing.sh` (103 cases —
...
matcher, and the session dropping mid-edit). 269 cases in all.
...
- `/bin/sh` being dash
- All 269 tests, and the same again under both sanitizers
...
- All 269 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
