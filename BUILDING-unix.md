...
matcher, and the session dropping mid-edit). 271 cases in all.
...
- `/bin/sh` being dash
- All 271 tests, and the same again under both sanitizers
...
- All 271 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
