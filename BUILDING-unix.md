...
`test` runs all five suites: `scripts/test-encoding.sh` (50 cases — kanji,
UTF-8, multi-byte editing, file names), `scripts/test-editing.sh` (76 cases —
...
matcher, and the session dropping mid-edit). 227 cases in all.
...
- `/bin/sh` being dash
- All 227 tests, and the same again under both sanitizers
...
- All 227 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
