...
`test` runs all five suites: `scripts/test-encoding.sh` (95 cases — kanji,
...
matcher, and the session dropping mid-edit). 331 cases in all.
...
- `/bin/sh` being dash
- All 331 tests, and the same again under both sanitizers
...
- All 331 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
