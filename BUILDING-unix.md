...
`test` runs all five suites: `scripts/test-encoding.sh` (101 cases — kanji,
...
matcher, and the session dropping mid-edit). 337 cases in all.
...
- `/bin/sh` being dash
- All 337 tests, and the same again under both sanitizers
...
- All 337 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
