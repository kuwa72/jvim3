...
UTF-8, multi-byte editing, file names), `scripts/test-editing.sh` (97 cases —
...
matcher, and the session dropping mid-edit). 263 cases in all.
...
- `/bin/sh` being dash
- All 263 tests, and the same again under both sanitizers
...
- All 263 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
