...
`test` runs all five suites: `scripts/test-encoding.sh` (54 cases — kanji,
...
matcher, and the session dropping mid-edit). 290 cases in all.
...
- `/bin/sh` being dash
- All 290 tests, and the same again under both sanitizers
...
- All 290 tests, and `-DTERMCAP` against base curses (found as `-lcurses`),
  with no warnings
